...
const Studio = () => {
    ...
    useGSAP(
        () => {
            ...
            const allParagraphs = gsap.utils.toArray('.containers .paragraph--animate');
        ...
        }, { scope: ".studio-container", dependencies: [] });
                ...
                <section className="containers mt-28 px-5 sm:px-10 md:px-16 space-y-20">
                    {container.length > 0 &&
                        container.map((container) => (
                            <section
                                key={container.id}
                                className={`container-${container.id} space-y-10 lg:space-y-28 `}
                            >
                                <div className={`overflow-hidden`}>
                                    <h2
                                        className={`heading--animate uppercase text-[${20 - container.heading.fontsize}vw] leading-[17vw] text-center`}
                                    >
                                        {container.heading.name}
                                    </h2>
                                </div>
                                <div className="grid grid-cols-1 gap-10 lg:grid-cols-5 lg:gap-0 max-w-[1145px] mx-auto">
                                    <p className="paragraph--animate text-2xl leading-[1.5rem] sm:text-3xl lg:text-4xl lg:max-w-[430px] lg:mx-auto lg:col-span-2">
                                        {container.text}
                                    </p>
                                    <ul className="capitalize text-lg font-light lg:col-start-4 col-span-full">
                                        {container.list.map((item, index) => (
                                            <li key={item} className="items--animate border-b-[1px] border-black/40 pb-5 mb-5 text-sm">
